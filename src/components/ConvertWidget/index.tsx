...
const Converter: FunctionComponent<Props> = ({
	...
	useEffect(() => {
		setMode(unit ? unit === "sasin" ? false : true : true);
    ...
    return <div className={style.converter}>
			<div className={style.converter__input}>
				<NumericField
					placeholder={hasFocused ? null : mode ? t("enterNumber") : t("enterSasinNumber")}
					onFocus={onFocus}
					onBlur={onBlur}
					onChange={onChange}
					value={value}/>
				<span className={style.converter__inputUnit}>{mode ? "" : value ? " sasinów" : ""}</span>
			</div>
			...
			<div className={style.converter__outputBox} onClick={onClipboard}>
				...
				<span className={style.converter__output}>
					...
					<span className={style.converter__outputUnit}>{mode ? " sasinów" : ""}</span>
				</span>
				<Icon className={style.converter__copyIcon} icon={faClipboard} size="2x"/>
			</div>
		</div>
};
